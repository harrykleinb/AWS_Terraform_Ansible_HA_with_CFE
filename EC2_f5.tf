...
resource "null_resource" "system_init_bigip1" {
  ...
  command = "ansible-playbook --user ${var.admin_user} --private-key ${var.private_key_path} ./PlayBook_Ansible_Reset_Pwd.yml -e \"bigip_ip=${data.aws_instance.bigip1.public_ip}\" -e 'admin_pwd=${var.bigip_admin_password}' -i ${data.aws_instance.bigip1.public_ip},"
...
resource "null_resource" "system_init_bigip2" {
  ...
  command = "ansible-playbook --user ${var.admin_user} --private-key ${var.private_key_path} ./PlayBook_Ansible_Reset_Pwd.yml -e \"bigip_ip=${data.aws_instance.bigip2.public_ip}\" -e 'admin_pwd=${var.bigip_admin_password}' -i ${data.aws_instance.bigip2.public_ip},"
...
resource "null_resource" "send_CFE_declaration_bigip2" {
  ...
  command = "ansible-playbook --user ${var.admin_user} --private-key ${var.private_key_path} ./PlayBook_Ansible_CFE.yml -e \"bigip=${data.aws_instance.bigip2.public_ip}\" -e user=${var.admin_user} -e 'password=${var.bigip_admin_password}' -e host=${var.host2} -e label=${var.cfe_label} -e ext_self_bigip1=${var.external_selfip_bigip1} -e ext_self_bigip2=${var.external_selfip_bigip2} -e log_level=silly -i ${data.aws_instance.bigip2.public_ip},"
...
resource "null_resource" "AS3_Declaration" {
  ...
  command = "ansible-playbook ./PlayBook_Ansible_AS3.yml -e \"bigip=${data.aws_instance.bigip1.public_ip}\" -e user=${var.admin_user} -e 'password=${var.bigip_admin_password}' -e Tenant_Name=Team_A -e App_Name=WebApp -e VIP=${var.IP_VS_1} -e Region=${var.aws_region} -i ${data.aws_instance.bigip1.public_ip},"
